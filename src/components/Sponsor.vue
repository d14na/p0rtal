<template>
    <div class="card tx-center mg-t-10">
        <div class="text-right pd-10">
            <a href="javascript://"><small>Sponsored Content <i class="fa fa-info-circle mg-r-5"></i></small></a>
        </div>

        <div class="card-body pd-20">
            <div class="d-flex justify-content-center mg-b-30">
                <img src="img/zerogold.png" class="wd-100 ht-100" alt="">
            </div>

            <h6 class="tx-md-20 tx-inverse mg-b-20">ZeroGold</h6>

            <p>
                Empowering a <u>fresh</u> generation of entreprenuers with the means to <strong>GROW</strong> their own DApps and <strong>HODL</strong> their own Wealth.
            </p>

            <a href="/zerogold.bit" class="btn btn-primary btn-block">Learn More</a>
        </div>
    </div>
</template>

<script>
export default {
    name: 'Sponsor',
    // props: {
    //     msg: String
    // }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
</style>
